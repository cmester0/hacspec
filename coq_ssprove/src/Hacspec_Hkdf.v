...
Definition hkdf_error_t : choice_type :=
  'unit.
  ...
  ((letbm salt_or_zero_898 : seq uint8 loc( salt_or_zero_898_loc ) :=
      ...
      letb '(salt_or_zero_898) :=
        if (seq_len (lift_to_both0 salt_900)) >.? (lift_to_both0 (
            usize 0)) :'bool
        then lift_scope (L1 := fset ([salt_or_zero_898_loc])) (L2 := fset (
...
Program Definition div_ceil (a_911 : uint_size) (b_912 : uint_size)
  ...
  ((letbm q_909 : uint_size loc( q_909_loc ) :=
      ...
      letb '(q_909) :=
        ...
        then lift_scope (L1 := fset ([q_909_loc])) (L2 := fset ([q_909_loc])) (
          I1 := [interface]) (
          I2 := [interface]) (H_loc_incl := _) (H_opsig_incl := _) (
...
Notation "'check_output_limit_inp'" :=(
  uint_size : choice_type) (in custom pack_type at level 2).
Notation "'check_output_limit_inp'" :=(uint_size : choice_type) (at level 2).
Notation "'check_output_limit_out'" :=((result (hkdf_error_t) (
      uint_size)) : choice_type) (in custom pack_type at level 2).
Notation "'check_output_limit_out'" :=((result (hkdf_error_t) (
      uint_size)) : choice_type) (at level 2).
Definition CHECK_OUTPUT_LIMIT : nat :=
  916.
Program Definition check_output_limit (l_914 : uint_size)
  : both (fset ([q_909_loc])) [interface] ((result (hkdf_error_t) (
        uint_size))) :=
...
Definition t_918_loc : Location :=
  (seq uint8 ; 919%nat).
Definition t_i_917_loc : Location :=
  (prk_t ; 920%nat).
Notation "'expand_inp'" :=(
...
Notation "'expand_out'" :=(hkdf_byte_seq_result_t : choice_type) (at level 2).
