...
[dependencies.hacspec-concordium-derive]
path = "../concordium-derive"
version = "2.0"
...
# wasm-test = ["hacspec-concordium-derive/wasm-test"]
# build-schema = ["hacspec-concordium-derive/build-schema"]
...
use hacspec_concordium_types::*; // hacspec_concordium_types::LogError, 
#[cfg(not(feature = "hacspec"))]
...
#[cfg(not(feature = "hacspec"))]
#[trusted]
pub(crate) fn invoke_creusot(tag: u32, start: *const u8, length: u32) -> u64 {
    unsafe { invoke(tag, start, length) }
}

#[cfg(feature = "hacspec")]
pub(crate) fn invoke_hacspec(tag: u32, start: PublicByteSeq) -> u64 {
    1u64
}
...
pub(crate) fn invoke_hacspec(tag: u32, start: PublicByteSeq) -> u64 {
    ...
    invoke_creusot(tag, temp.as_ptr(), start.len() as u32)
}
...
#[cfg(not(feature = "hacspec"))]
#[trusted]
pub(crate) fn write_output_creusot(start: *const u8, length: u32, offset: u32) -> u32 {
    unsafe { write_output(start, length, offset) }
}

#[cfg(feature = "hacspec")]
pub(crate) fn write_output_hacspec(start: PublicByteSeq, offset: u32) -> u32 {
    1u32
}
...
pub(crate) fn write_output_hacspec(start: PublicByteSeq, offset: u32) -> u32 {
    let temp = &mut coerce_hacspec_to_rust_public_byte_seq(start.clone())[..];
    write_output_creusot(temp.as_ptr(), start.len() as u32, offset)
}

#+end_src
Extern get parameter size
#+begin_src rust
#[cfg(not(feature = "hacspec"))]
extern "C" {
...
#[cfg(not(feature = "hacspec"))]
#[trusted]
pub(crate) fn get_parameter_size_creusot(i : u32) -> i32 {
    unsafe { get_parameter_size(i) }
}

#[cfg(feature = "hacspec")]
pub(crate) fn get_parameter_size_hacspec(i : u32) -> i32 {
    1i32
}

#[cfg(not(feature = "hacspec"))]
pub(crate) fn get_parameter_size_hacspec(i : u32) -> i32 {
    get_parameter_size_creusot(i)
}
...
#[cfg(not(feature = "hacspec"))]
#[trusted]
pub(crate) fn get_parameter_section_creusot(i: u32, param_bytes: *mut u8, length: u32, offset: u32) -> i32 {
    unsafe { get_parameter_section(i, param_bytes, length, offset) }
}

#[cfg(feature = "hacspec")]
pub(crate) fn get_parameter_section_hacspec(i : u32, buf: PublicByteSeq, offset: u32) -> (PublicByteSeq, i32) {
    (buf, 1i32)
}
...
pub(crate) fn get_parameter_section_hacspec(i : u32, buf: PublicByteSeq, offset: u32) -> (PublicByteSeq, i32) {
    ...
    let result = get_parameter_section_creusot(i, temp.as_mut_ptr(), buf.len() as u32, offset);
    (
...
#+begin_src rust
  ...
  #[cfg(feature = "hacspec")]
  pub(crate) fn log_event_hacspec(start: PublicByteSeq) -> i32 {
      1i32
  }
  ...
  pub(crate) fn log_event_hacspec(start: PublicByteSeq) -> i32 {
      let temp = &mut coerce_hacspec_to_rust_public_byte_seq(start.clone())[..];
      log_event_creusot(temp.as_ptr(), start.len() as u32)
  }
...
#+end_src
Extern get receive self address
#+begin_src rust
  #[cfg(not(feature = "hacspec"))]
  extern "C" {
...
#+end_src
Extern get receive self balance
#+begin_src rust
  #[cfg(not(feature = "hacspec"))]
  extern "C" {
...
#+end_src
Extern get receive owner (unused)
...
*** Return value
#+begin_src rust
  pub type ReturnValueHacspec = u32;  
#+end_src

#+begin_src rust
pub fn return_value_impl_write(
    current_position: ReturnValueHacspec,
    buf: PublicByteSeq,
) -> Result<(ReturnValueHacspec, usize), ()> {
    if current_position.checked_add(buf.len() as u32).is_none() {
        Result::<(ReturnValueHacspec, usize), ()>::Err(())?;
    }
    let num_bytes = write_output_hacspec(buf, current_position);
    Result::<(ReturnValueHacspec, usize), ()>::Ok((
        current_position + (num_bytes as u32),
        num_bytes as usize,
    ))
}
#+end_src
...
#+begin_src rust
#[cfg(not(feature = "hacspec"))]
pub fn coerce_rust_to_hacspec_return_value(
    rust_return_value: &mut ReturnValue,
) -> ReturnValueHacspec {
    rust_return_value.current_position.clone()
}

#[cfg(not(feature = "hacspec"))]
pub fn coerce_hacspec_to_rust_return_value(
    rust_return_value: &mut ReturnValue,
    hacspec_return_value: ReturnValueHacspec,
) {
    rust_return_value.current_position = hacspec_return_value;
}
#+end_src
...
impl Write for ReturnValue {
    ...
    fn write(&mut self, buf: &[u8]) -> Result<usize, Self::Err> {
        let (cs, nr) = return_value_impl_write(
            coerce_rust_to_hacspec_return_value(self),
            coerce_rust_to_hacspec_public_byte_seq(buf),
        )?;
        coerce_hacspec_to_rust_return_value(self, cs);
        Ok(nr)
    }
}

// TODO : Hacspec implementation
#[cfg(not(feature = "hacspec"))]
impl ReturnValue {
    #[inline(always)]
    pub fn open() -> Self {
...
We define parameter
#+begin_src rust
pub type ExternParameterHacspec = u32;

pub fn read_impl_for_extern_parameter_read(
    current_position: ExternParameterHacspec,
    buf: PublicByteSeq,
) -> (ExternParameterHacspec, usize) {
    let (_buf, num_read) = get_parameter_section_hacspec(0u32, buf, current_position);
    (current_position + (num_read as u32), num_read as usize)
}
...
#+begin_src rust
#[cfg(not(feature = "hacspec"))]
pub fn coerce_rust_to_hacspec_extern_parameter(
    rust_parameter: &mut ExternParameter,
) -> ExternParameterHacspec {
    rust_parameter.current_position.clone()
}

#[cfg(not(feature = "hacspec"))]
pub fn coerce_hacspec_to_rust_extern_parameter(
    rust_parameter: &mut ExternParameter,
    hacspec_parameter: ExternParameterHacspec,
) {
...
impl Read for ExternParameter {
    fn read(&mut self, buf: &mut [u8]) -> ParseResult<usize> {
        let (cs, nr) = read_impl_for_extern_parameter_read(
            coerce_rust_to_hacspec_extern_parameter(self),
            coerce_rust_to_hacspec_public_byte_seq(buf),
        );
        coerce_hacspec_to_rust_extern_parameter(self, cs);
        Ok(nr)
    }
}
...
impl HasParameter for ExternParameter {
    #[inline(always)]
    fn size(&self) -> u32 {
        get_parameter_size_hacspec(0) as u32
    }
}
#+end_src
...
#+begin_src rust
// #[derive(Debug)]
pub type CallResponseHacspec = (u32, u32);
// pub struct CallResponse {
//     /// The index of the call response.
//     pub(crate) i:                NonZeroU32,
//     pub(crate) current_position: u32,
...
#[cfg(not(feature = "hacspec"))]
pub fn coerce_rust_to_hacspec_call_response(
    rust_call_response: &mut CallResponse,
) -> CallResponseHacspec {
    (rust_call_response.i.get(), rust_call_response.current_position.clone())
}

#[cfg(not(feature = "hacspec"))]
pub fn coerce_hacspec_to_rust_call_response(
    rust_call_response: &mut CallResponse,
    hacspec_call_response: CallResponseHacspec,
) {
    let (i, current_position) = hacspec_call_response;
    rust_call_response.i = unsafe { NonZeroU32::new_unchecked(i) }; // TODO: Move unsafe to its own function
    rust_call_response.current_position = current_position;
}


/// Read a u32 in little-endian format. This is optimized to not
/// initialize a dummy value before calling an external function.
pub fn call_response_impl_read(
    call_response: CallResponseHacspec,
) -> (CallResponseHacspec, Result<usize, ()>) {
    // let mut bytes: MaybeUninit<[u8; 8]> = MaybeUninit::uninit();
    let (i, current_position) = call_response;
    let buf = PublicByteSeq::new(8);
    let (buf, num_read) = get_parameter_section_hacspec(i as u32, buf, current_position);
    ((i, current_position + (num_read as u32)), // TODO: should i be updated? ,, if num_read >= 0i32 { should be added to current_position + num_read
     if num_read >= 0i32 {
         Result::<usize, ()>::Ok(num_read as usize)
     } else {
         Result::<usize, ()>::Err(())
     }) // num_read as u64
}
...
impl Read for CallResponse {
    fn read(&mut self, buf: &mut [u8]) -> ParseResult<usize> {
        let (cs, nr) =
            call_response_impl_read(coerce_rust_to_hacspec_call_response(self));
        coerce_hacspec_to_rust_call_response(self, cs);
        match nr {
            Result::<usize, ()>::Ok(a) => ParseResult::<usize>::Ok(a),
            Result::<usize, ()>::Err(_) => ParseResult::<usize>::Err(ParseError::default()),
        }
    }
}
...
impl HasCallResponse for CallResponse {
    fn size(&self) -> u32 { get_parameter_size_hacspec(self.i.get()) as u32 }
}
...
const INVOKE_TRANSFER_TAG: u32 = 0u32;
const INVOKE_CALL_TAG: u32 = 1u32;

pub enum InvokeErrorHacspec {
    LogicReject(i32, u32, u32),
    AmountTooLarge,
    MissingAccount,
    MissingContract,
    MissingEntrypoint,
...
// TODO: pearlite guarantee #[ensures(exists<b : bool> exists<v : u32> result === Ok((b, v)) && !(v === 0))]
fn parse_response_code_hacspec(code: u64) -> Result<(bool, u32), InvokeErrorHacspec> {
    let mut result = Result::<(bool, u32), InvokeErrorHacspec>::Err(
        InvokeErrorHacspec::IsTrap,
    );
    
    if code & !0xffff_ff00_0000_0000u64 == 0u64 {
        // this means success
        let rv = (code >> 40) as u32;
        let tag = 0x80_0000u32;
        if tag & rv != 0u32 {
            result = Result::<(bool, u32), InvokeErrorHacspec>::Ok((true, rv & !tag))
        } else {
            result = Result::<(bool, u32), InvokeErrorHacspec>::Ok((false, rv))
        }
    } else {
        let match_val = (0x0000_00ff_0000_0000u64 & code) >> 32;
        if match_val == 0x00u64
        // response with logic error and return value.
        {
            let reason = (0x0000_0000_ffff_ffffu64 & code) as u32 as i32;
            if reason == 0i32 {
                result = Result::<(bool, u32), InvokeErrorHacspec>::Err(
                    InvokeErrorHacspec::IsTrap,
                )
                ...
                let rv = (code >> 40) as u32;
                if rv > 0u32 {
                    result = Result::<(bool, u32), InvokeErrorHacspec>::Err(InvokeErrorHacspec::LogicReject(
                        reason, rv, 0u32,
                    ))
                } else {
                    result = Result::<(bool, u32), InvokeErrorHacspec>::Err(
                    ...
                    // trap() // crate:: // host precondition violation.
                }
            }
        } else {
            if match_val == 0x01u64 {
                result = Result::<(bool, u32), InvokeErrorHacspec>::Err(InvokeErrorHacspec::AmountTooLarge)
            } else {
                if match_val == 0x02u64 {
                    result = Result::<(bool, u32), InvokeErrorHacspec>::Err(
                        InvokeErrorHacspec::MissingAccount,
                    )
                } else {
                    if match_val == 0x03u64 {
                        result = Result::<(bool, u32), InvokeErrorHacspec>::Err(
                            InvokeErrorHacspec::MissingContract,
                        )
                    } else {
                        if match_val == 0x04u64 {
                            result = Result::<(bool, u32), InvokeErrorHacspec>::Err(
                                InvokeErrorHacspec::MissingEntrypoint,
                            )
                        } else {
                            if match_val == 0x05u64 {
                                result = Result::<(bool, u32), InvokeErrorHacspec>::Err(
                                    InvokeErrorHacspec::MessageFailed,
                                )
                            } else {
                                if match_val == 0x06u64 {
                                    result = Result::<(bool, u32), InvokeErrorHacspec>::Err(
                                        InvokeErrorHacspec::Trap,
                                    )
                                } else {
                                    result = Result::<(bool, u32), InvokeErrorHacspec>::Err(
                                        InvokeErrorHacspec::Unknown,
                                    )
                                } // FIXME: This should be trap as well.
                            }
                        }
                    }
                }
...
fn parse_response_code(code: u64) -> InvokeResult<(bool, Option<NonZeroU32>)> {
    match parse_response_code_hacspec(code) {
        Ok((a, b)) => Ok((a, NonZeroU32::new(b))),
        Err(err) => Err(match err {
            InvokeErrorHacspec::LogicReject(reason, rv, cp) => InvokeError::LogicReject {
                reason,
                return_value: CallResponse {
                    i: unsafe { NonZeroU32::new_unchecked(rv) },
                    current_position: cp,
                },
            }, // TODO: Move unsafe to its own function
            InvokeErrorHacspec::AmountTooLarge => InvokeError::AmountTooLarge,
            InvokeErrorHacspec::MissingAccount => InvokeError::MissingAccount,
            InvokeErrorHacspec::MissingContract => InvokeError::MissingContract,
            InvokeErrorHacspec::MissingEntrypoint => InvokeError::MissingEntrypoint,
            InvokeErrorHacspec::MessageFailed => InvokeError::MessageFailed,
            InvokeErrorHacspec::Trap => InvokeError::Trap,
            InvokeErrorHacspec::Unknown => InvokeError::Unknown,
            InvokeErrorHacspec::IsTrap => trap(),
        }),
    }
}
...
pub fn host_impl_has_host_state_invoke_transfer(receiver: UserAddress, amount: u64) -> u64 {
    let data = PublicByteSeq::new(0)
        .concat(&receiver)
        .concat(&u64_to_be_bytes(amount)); // Should end at length 'ACCOUNT_ADDRESS_SIZE + 8'
    let response = invoke_hacspec(INVOKE_TRANSFER_TAG, data); // length data == (ACCOUNT_ADDRESS_SIZE + 8) as u32
    response
}
...
impl<State: Deserial + Serial> HasHost<State> for Host<State> {
    ...
    fn invoke_transfer(&mut self, receiver: &AccountAddress, amount: Amount) -> InvokeResult<()> {
        let response = host_impl_has_host_state_invoke_transfer(
            coerce_rust_to_hacspec_account_address(receiver),
            amount.micro_ccd,
        );
...
//                     Err(_) => trap(), // crate::
//                 },
//                 _ => trap(), // crate:: // unreachable!("Host violated precondition."),
//             }
//         }
//     }
...
#+begin_src rust
  ...
  impl HasLogger for Logger {
      ...
      fn log_raw(&mut self, event: &[u8]) -> Result<(), LogError> {
          match log_event_hacspec(coerce_rust_to_hacspec_public_byte_seq(event)) {
              1 => Ok(()),
              0 => Err(LogError::Full),
