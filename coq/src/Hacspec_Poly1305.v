...
Next Obligation.
  intros.
  ssprove_valid ;
    try repeat rewrite choice_type_from_type_id ;
    try repeat rewrite type_from_choice_type_id ;
    try (apply valid_scheme ; apply prog_valid) ;
    try (apply eqtype.predU1l; reflexivity).
Defined.
...
Program Definition poly1305_init
  ...
  ({code
     temp_38 ←
      (array_from_slice (default) (16) (k_36) (usize 0) (usize 16)) ;; 
...
Next Obligation.
  intros.
  destruct (type_from_choice_type_elem st_69) as [ [a b] c ].
  ssprove_valid ;
    try repeat rewrite choice_type_from_type_id ;
    try repeat rewrite type_from_choice_type_id ;
    try (apply valid_scheme ; apply prog_valid) ;
...
Next Obligation.
  ...
  ssprove_valid ;
    ...
    try (apply valid_scheme ; apply prog_valid) ;
    try (apply eqtype.predU1l; reflexivity).
  destruct (type_from_choice_type_elem v) as [ [a b] c ].
  ssprove_valid ;
    try repeat rewrite choice_type_from_type_id ;
    try repeat rewrite type_from_choice_type_id ;
    try (apply valid_scheme ; apply prog_valid) ;
...
Next Obligation.
  ...
  ssprove_valid ;
    ...
    try (apply valid_scheme ; apply prog_valid) ;
    try (apply eqtype.predU1l; reflexivity).
  all: try (apply eqtype.predU1r; apply eqtype.predU1l; reflexivity).
  destruct (type_from_choice_type_elem v0) as [ [a b] c ].
  ssprove_valid ;
    try repeat rewrite choice_type_from_type_id ;
    try repeat rewrite type_from_choice_type_id ;
    try (apply valid_scheme ; apply prog_valid) ;
...
Next Obligation.
  ...
  ssprove_valid ;
    ...
    try (apply valid_scheme ; apply prog_valid) ;
    try (apply eqtype.predU1l ; reflexivity) ;
    try (repeat (apply eqtype.predU1r ; (try (apply eqtype.predU1l ; reflexivity)))).
  - destruct (type_from_choice_type_elem v1) as [ [a b] c ].
    ssprove_valid ;
      try repeat rewrite choice_type_from_type_id ;
      try repeat rewrite type_from_choice_type_id ;
      try (apply valid_scheme ; apply prog_valid) ;
      try (apply eqtype.predU1l ; reflexivity) ;
      try (repeat (apply eqtype.predU1r ; (try (apply eqtype.predU1l ; reflexivity)))).
  - destruct (type_from_choice_type_elem x0) as [ [a b] c ].
    ssprove_valid ;
      try repeat rewrite choice_type_from_type_id ;
      try repeat rewrite type_from_choice_type_id ;
      try (apply valid_scheme ; apply prog_valid) ;
