...
Proof.
  ...
  apply (@rew_opp_r Type (fun x => x) (Choice.sort (chElement (@ct _))) (@T _)).
Qed.
...
Proof.
  ...
  apply (@rew_opp_l Type (fun x => x) (Choice.sort (chElement (@ct _))) (@T _)).
Qed.
...
Proof.
  intros.
  pose proof (Z_mod_two_p_range wordsize x).
  destruct x.
  - easy.
  - easy.
  - unfold Z_mod_modulus.
    unfold Z_mod_two_p in H.
    destruct (zeq (P_mod_two_p p wordsize) 0) ; easy.
Qed.
...
Proof.
  intros.
  pose proof (Z_mod_two_p_eq wordsize x).
  destruct x.
  - easy.
  - easy.
  - unfold Z_mod_modulus.
...
Definition unsigned (n: int) : Z := Z.of_nat (nat_of_ord n).
...
Lemma mkint_eq:
  forall x y Px Py, x = y -> Ordinal (n := Z.to_nat modulus) (m := x) Px = Ordinal (n := Z.to_nat modulus) (m := y) Py.
Proof.
  intros.
  subst.
...
Definition destruct_int_range (i : int) : -1 < Z.of_nat (nat_of_ord i) < modulus.
Proof.
  pose (@ltn_ord (Z.to_nat modulus) i).
  split.
  - lia.
  - rewrite <- Z2Nat.id.
    apply inj_lt.
    ...
    + easy.
Qed.
(* Definition destruct_int (i : int) : i = mkint (Z.of_nat (nat_of_ord i)) (destruct_int_range i). *)
(* Proof. *)
(*   destruct i ; unfold pos in *. *)
(*   fold modulus. *)
...
Proof.
  intros.
  pose proof (destruct_int_range x).
  pose proof (destruct_int_range y).
  destruct x; destruct y ; unfold pos in *.
  
  cbn in H, H0.
  generalize dependent m ; intros m ; rewrite <- (Nat2Z.id m) ; intros.
  generalize dependent m0 ; intros m0 ; rewrite <- (Nat2Z.id m0) ; intros.
  (* cbn in H, H0. *)
  
  destruct (m =? m0)%nat eqn:meq ; [apply Nat.eqb_eq in meq ; subst | apply Nat.eqb_neq in meq ; subst].
  - left. apply mkint_eq. reflexivity.
  - right.
...
Definition add_overflow (x y cin: int): int :=
  let s := (signed x + signed y + signed cin)%Z in
...
Remark half_modulus_modulus: modulus = (2 * half_modulus)%Z.
Proof.
...
Proof.
  intros. simpl. unfold unsigned. unfold repr. cbn. rewrite Z2Nat.id.
  apply Z_mod_modulus_eq.
  apply Z_mod_modulus_range.
Qed.
...
Proof.
  intros i.
  pose (destruct_int_range i).
  destruct i ; unfold pos in *. unfold unsigned in *. cbn in *. lia.
Qed.
...
Proof.
  ...
  unfold max_signed. generalize min_signed_neg. cbn. lia.
  unfold min_signed, max_signed.
  (* cbn. *)
  rewrite half_modulus_modulus in H.
  cbn.
...
Proof.
  intros i.
  pose proof (destruct_int_range i).
  destruct i; unfold pos in * ; unfold repr. unfold unsigned.
  cbn in H.
  
  apply mkint_eq.

  rewrite Z_mod_modulus_eq. cbn. rewrite Z.mod_small. rewrite Nat2Z.id. reflexivity.
  lia.
Qed.
...
Proof.
  ...
  destruct (zlt (unsigned x) half_modulus); cbn ; lia.
Qed.
...
Proof.
  ...
  replace (-1) with ((modulus - 1) + (-1) * modulus)%Z.
...
Proof.
  ...
  - unfold proj_sumbool. rewrite e. rewrite zeq_true. auto.
  - unfold proj_sumbool. rewrite zeq_false. auto. auto.
Qed.

Theorem eq_spec: forall (x y: int), if eq x y then x = y else x <> y.
Proof.
  intros; unfold eq. case (eq_dec x y); intro.
  - subst y. unfold proj_sumbool. rewrite zeq_true. auto.
...
Proof.
  ...
  subst x. unfold proj_sumbool.  rewrite zeq_true; auto.
...
Proof.
  ...
  replace 0 with (unsigned x + (- (unsigned x)))%Z.
  auto with ints. lia.
Qed.
...
Corollary unsigned_add_either:
  forall x y,
  unsigned (add x y) = (unsigned x + unsigned y)%Z
...
Theorem unsigned_sub_borrow:
  forall x y,
  unsigned (sub x y) = (unsigned x - unsigned y + unsigned (sub_borrow x y zero) * modulus)%Z.
Proof.
  intros.
...
Proof.
  ...
  replace (unsigned x * (modulus - 1))%Z with (unsigned x * modulus - unsigned x)%Z by ring.
...
Proof.
  ...
  set (y' := unsigned y).
  set (z' := unsigned z).
  apply eqm_samerepr. apply eqm_trans with ((x' * y') * z')%Z.
  auto with ints.
...
Proof.
  ...
  apply eqm_trans with ((x' + y') * z')%Z.
  auto with ints.
  replace ((x' + y') * z')%Z with (x' * z' + y' * z')%Z.
  auto with ints.
  ring.
Qed.
...
Proof.
  ...
  apply eqm_samerepr.
  set (x' := unsigned x). set (y' := unsigned y).
  apply eqm_trans with ((x' / y') * y' + x' mod y')%Z.
...
Proof.
  ...
  apply eqm_trans with ((Z.quot x' y') * y' + Z.rem x' y')%Z.
...
Proof.
  ...
  set (N := (unsigned zero * modulus + unsigned n)%Z).
...
Lemma unsigned_signed:
  forall n, unsigned n = if lt n zero then (signed n + modulus)%Z else signed n.
Proof.
...
- unfold proj_sumbool. rewrite zlt_false ; [ | lia]. auto.
- unfold proj_sumbool. rewrite zlt_true  ; [ | lia]. ring.
Qed.
...
Proof.
  ...
  unfold proj_sumbool. rewrite dec_eq_false ; [ | auto].
  set (N := (signed (if lt n zero then mone else zero) * modulus + unsigned n)%Z).
  set (D := signed d).
  assert (D <> 0).
  ...
  - (* D = -1 *)
    rewrite e. change (-1) with (Z.opp 1). rewrite Z.quot_opp_r ; [ | lia].
    rewrite Z.quot_1_r.
    assert (N <> min_signed).
  ...
  - (* |D| > 1 *)
    ...
    { rewrite <- Z.quot_abs by lia. apply Zquot_lt_upper_bound.
      extlia. extlia.
      apply Z.le_lt_trans with (half_modulus * 1)%Z.
  ...
  unfold proj_sumbool; rewrite ! zle_true ; [ | lia | lia]; simpl.
...
Proof.
  ...
  assert (half_modulus = two_power_nat ws1).
    ...
    unfold proj_sumbool.
    rewrite Zsign_bit. destruct (zlt (unsigned x) (two_power_nat ws1)) ; reflexivity.
    rewrite two_power_nat_S. rewrite <- H0.
...
Proof.
  ...
  - apply same_bits_eqm. apply eqm_signed_unsigned. unfold proj_sumbool. lia.
  - unfold signed. rewrite sign_bit_of_unsigned. destruct (zlt (unsigned x) half_modulus).
    ...
    + apply Ztestbit_above_neg with wordsize.
      fold modulus. generalize (unsigned_range x).  unfold proj_sumbool. lia. auto.
Qed.
...
Proof.
  ...
  replace (-unsigned x - 1) with (-unsigned x + (-1))%Z by lia.
  apply eqm_add.
...
Proof.
  ...
  destruct (zlt (unsigned x - unsigned y - unsigned b)).
   unfold proj_sumbool. rewrite zlt_true. rewrite xor_zero_l; auto.
  unfold max_unsigned; lia.
   unfold proj_sumbool. rewrite zlt_false. rewrite xor_idem; auto.
  unfold max_unsigned; lia.
  destruct H; subst b.
  rewrite xor_zero_l. rewrite unsigned_one. rewrite unsigned_zero; auto.
  rewrite xor_idem. rewrite unsigned_one. rewrite unsigned_zero; auto.
Qed.
...
Lemma bits_shr:
  ...
  0 <= i < zwordsize ->
  testbit (shr x y) i =
  testbit x (if zlt (i + unsigned y) zwordsize then (i + unsigned y)%Z else zwordsize - 1).
Proof.
...
Proof.
  bit_solve. rewrite unsigned_zero. unfold proj_sumbool. rewrite zlt_false. f_equal; lia. lia.
Qed.
...
Proof.
  intros. unfold shrx. rewrite shl_zero. unfold divs. rewrite signed_one ; [ | auto].
