...
(* Definition field_element_t := nat_mod 0x03fffffffffffffffffffffffffffffffb. *)
  ...
  Definition P := 0x3fffffffffffffffffffffffffffffffb.
  Definition gT := Zp_baseFinGroupType P.-1.
  ...
  Definition Plain : finType :=
    FinGroup.arg_finType (gT).
  Definition Plain_pos : Positive #|Plain| := _.               
    
  Definition chPlain := 'fin #|Plain|.

  Definition field_element_t := 'fin #|Plain|. (* 'fin 0x03fffffffffffffffffffffffffffffffb. *)
  Definition elem_to_nat_mod : field_element_t -> nat_mod (Z.of_nat (#|Plain|)).
  Proof.
    intros t.
    unfold field_element_t in t.

    unfold Plain in *.
    apply (nat_mod_from_chFin t).
  Qed.
  Global Coercion nat_mod_to_elem (t : nat_mod (Z.of_nat (#|Plain|))) : field_element_t.
  Proof.
    assert (Z.of_nat #|gT| > 0).
    {
      rewrite card_ord.
      easy.
    }    
    apply (@chFin_from_nat_mod #|gT| H t).
  Qed.
...
Definition poly1305_encode_r (b_0 : poly_block_t) : field_element_t :=
  ...
  let n_1 :=
    (n_1) .& (secret (
        @MachineIntegers.repr WORDSIZE128 21267647620597763993911028882763415551) : int128) in 
  nat_mod_to_elem (nat_mod_from_secret_literal (n_1)).
...
Definition poly1305_encode_block (b_2 : poly_block_t) : field_element_t :=
  ...
  let f_4 : field_element_t :=
    nat_mod_to_elem (nat_mod_from_secret_literal (n_3)) in 
  nat_mod_to_elem ((elem_to_nat_mod f_4) +% (nat_mod_pow2 (Z.of_nat #|Plain|) (
      usize 128))).
...
Definition poly1305_encode_last
  ...
  let f_8 : field_element_t :=
    nat_mod_to_elem (nat_mod_from_secret_literal (n_7)) in 
  nat_mod_to_elem ((elem_to_nat_mod f_8) +% (nat_mod_pow2 (Z.of_nat #|Plain|) ((usize 8) * (
        pad_len_5)))).
...
Definition poly1305_init (k_9 : poly_key_t) : poly_state_t :=
  let r_10 : field_element_t :=
    poly1305_encode_r (array_from_slice (default) (16) (k_9) (usize 0) (
        usize 16)) in 
  (nat_mod_to_elem nat_mod_zero , r_10, k_9).
...
Definition poly1305_update_block
  ...
  let '(acc_13, r_14, k_15) :=
    st_12 in 
  (nat_mod_to_elem (((elem_to_nat_mod (poly1305_encode_block (b_11))) +% (elem_to_nat_mod acc_13)) *% (elem_to_nat_mod r_14)), r_14, k_15).
...
Definition poly1305_update_last
  ...
  let '(st_25) :=
    if (seq_len (b_23)) !=.? (usize 0):bool then (let '(acc_26, r_27, k_28) :=
      ...
      let st_25 :=
        (
          nat_mod_to_elem ((elem_to_nat_mod (poly1305_encode_last (pad_len_22) (b_23)) +% (elem_to_nat_mod acc_26)) *% (elem_to_nat_mod r_27)),
          r_27,
          k_28
        ) in 
      (st_25)) else ((st_25)) in 
...
Definition poly1305_finish (st_33 : poly_state_t) : poly1305_tag_t :=
  ...
  let aby_37 : seq uint8 :=
    nat_mod_to_byte_seq_le (elem_to_nat_mod acc_34) in 
  let a_38 : uint128 :=
