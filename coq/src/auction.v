...
Definition auction_bid
  ...
  let '(
    ...
    | NotSoldYet => match (if ((slot_time_23) <=.? (expiry_21)) then (
    ...
    | Some sender_address_32 => match seq_map_entry (
    ...
    | Update (updated_bid_35, updated_map_36) => match (if (
      (updated_bid_35) >.? (b_19)) then (True) else (False)) with
    | True => (
...
| AuctionFinalized_ : finalize_error.
...
Definition auction_finalize
  ...
  let '(continues_48, return_action_49) :=
    ...
    | NotSoldYet => if ((slot_time_45) >.? (expiry_42)) then (
      if ((balance_47) =.? (repr 0)) then (
  ...
  let '(return_action_49) :=
    if continues_48 then (
      let return_action_49 :=
        ...
        | Some (amount_59, _) => match (if ((amount_59) =.? (b_40)) then (
          True) else (False)) with
