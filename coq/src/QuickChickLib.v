...
Definition g_int8 : G (int8) :=
  bindGen (* arbitrary *) (GenLow.choose (0,1000)) (fun x => returnGen (pub_u8 x)).
...
Definition g_int32 : G (int32) := (* restricted *)
  bindGen (GenLow.choose (0,1000)) (fun x => returnGen (pub_u32 x)).
...
Definition g_int64 : G (int64) :=
  bindGen (* arbitrary *) (GenLow.choose (0,1000)) (fun x => returnGen (pub_u64 x)).
...
(* #[global] Instance show_nseq {A} `{Show A} `{Default A} n : Show (nseq A n) := *)
(*   Build_Show (nseq A n) (fun x => show (array_to_list x)). *)
