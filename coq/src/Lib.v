...
Definition uint_size := int32. (* ordinal_choiceType 32. (* int32. *) *)
...
Global Coercion array_from_seq_3
  {a: choice_type}
 `{Default a}
  (input: seq a)
  (* {H : List.length input = out_len} *)
  : nseq a (usize (List.length input)) :=
  nseq_to_nseq_nat (array_from_seq_nat (List.length input) input).
...
(* Global Coercion array_from_seq2 *)
(*   {a: choice_type} *)
(*  `{Default a} *)
(*   {out_len:nat} *)
(*   (input: nseq a (usize out_len)) *)
...
Definition array_from_slice
 ...
 `{Default a}
  ...
  (slice_len: uint_size)
    : nseq a (usize out_len) :=
  nseq_to_nseq_nat (
      array_from_slice_nat default_value out_len
                           (seq_from_array input)
                           (from_uint_size start)
                           (from_uint_size slice_len)).
