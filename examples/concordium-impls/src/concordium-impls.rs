...
#[cfg(not(feature = "hacspec"))]
#[derive(Eq, PartialEq)] // TODO: Creusot issue readd "Debug" attribute 
#[repr(transparent)]
pub struct Reject {
...
impl HasInitContext for ExternContext<InitContextExtern> {
	...
	// let mut address : [u8; ACCOUNT_ADDRESS_SIZE] = Default::default(); // : [u8; ACCOUNT_ADDRESS_SIZE]
	let temp = coerce_hacspec_to_rust_public_byte_seq(get_init_origin_hacspec(
	    PublicByteSeq::new(ACCOUNT_ADDRESS_SIZE),
	));

	let address: [u8; ACCOUNT_ADDRESS_SIZE] = temp.try_into().unwrap(); //  {
	//     Ok(ba) => ba,
	//     Err(_) => panic!(),
	// }

	// address.clone_from_slice(temp_vec.as_slice());
