...
Definition bar  : (result int64 uint8) :=
  result_bind (foo (false)) (fun x_1 => @Ok int64 uint8 ((x_1) .+ (
        @repr WORDSIZE64 1))).

Definition fizzbaz  : (result int64 uint8) :=
  result_bind (foo (false)) (fun x_2 => result_bind (foo (true)) (fun y_3 =>
      @Ok int64 uint8 ((x_2) .+ (y_3)))).

Definition fizzbazbaz  : (result int64 uint8) :=
  result_bind (foo (false)) (fun x_4 => result_bind (foo (true)) (fun y_5 =>
      let x_4 :=
        (x_4) .+ (y_5) in 
      result_bind (foo (false)) (fun y_5  => @Ok int64 uint8 ((x_4) .+ (
            y_5))))).

Definition fizzbazbazbar (s_6 : seq int64) : (result int64 uint8) :=
  result_bind (foo (false)) (fun y_7 => result_bind (foo (true)) (fun _temp =>
      let s_6 :=
        seq_upd s_6 (usize 0) _temp in 
      @Ok int64 uint8 ((seq_index (s_6) (usize 0)) .+ (y_7)))).

Definition baz  : (result int32 uint8) :=
  result_bind (foo (false)) (fun x_8 => let out_9 : int32 :=
      @repr WORDSIZE32 0 in 
    ifbnd (true) || (false) : bool
    thenbnd (result_bind (foo (true)) (fun y_10 => result_bind (foo ((
              false) || (true))) (fun _ => Ok ((out_9)))))
    elsebnd(result_bind (foo ((false) && (true))) (fun _ => let out_9 :=
          (@cast _ uint32 _ (x_8)) .+ (@repr WORDSIZE32 1) in 
        Ok ((out_9)))) >> (fun '(out_9) =>
    @Ok int32 uint8 (out_9))).

Definition fizzbar  : (result int32 uint8) :=
  result_bind (foo (false)) (fun x_11 => let out_12 : int32 :=
      @repr WORDSIZE32 0 in 
    result_bind (foldibnd (usize 0) to (
        usize 200) for out_12>> (fun i_13 out_12 =>
      result_bind (foo (true)) (fun y_14 => let out_12 :=
...
Definition fizzbarbuzz  : (result int32 uint8) :=
  result_bind (foo (false)) (fun x_15 => let out_16 : int32 :=
      @repr WORDSIZE32 0 in 
    result_bind (foldibnd (usize 0) to (
        usize 200) for out_16>> (fun i_17 out_16 =>
      ifbnd (true) || (false) : bool
      thenbnd (result_bind (foo (true)) (fun y_18 => let out_16 :=
            (@cast _ uint32 _ (y_18)) .+ (out_16) in 
          result_bind (foo ((false) || (true))) (fun _ => Ok ((out_16)))))
      elsebnd(result_bind (foo ((false) && (true))) (fun _ => let out_16 :=
