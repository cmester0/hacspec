...
fn make_paren<'a>(e: RcDoc<'a, ()>) -> RcDoc<'a, ()> {
    RcDoc::as_string("(")
        .append(e.group().nest(2)) // RcDoc::line_().append(e)
        .append(RcDoc::as_string(")"))
        .group()
}
...
fn array_or_seq<'a>(t: Typ, top_ctx: &'a TopLevelContext) -> RcDoc<'a, ()> {
    match &(t.1).0 {
        ...
        BaseTyp::Named(id, None) => {
            ...
            match top_ctx.typ_dict.get(name) {
                Some((new_t, dict_entry)) => match dict_entry {
                    DictEntry::Alias => array_or_seq(new_t.clone(), top_ctx),
...
//     match l {
//         syn::Lit::Int(lit) => RcDoc::as_string(format!("{}", lit.base10_digits())),
//         syn::Lit::Bool(lit) => RcDoc::as_string(format!("{}", lit.value())),
//         _ => RcDoc::as_string(format!("TODO: {:?}", l)),
//     }
// }

// fn translate_pearlite<'a>(
//     t: pearlite_syn::term::Term,
//     top_ctx: &'a TopLevelContext,
//     idents: Vec<(String, Ident)>,
// ) ->  {
...
//             // make_paren(translate_pearlite(*left, top_ctx, idents.clone()))
//             //     .append(RcDoc::space())
//             //     .append(translate_pearlite_binop(op))
//             //     .append(RcDoc::space())
//             //     .append(make_paren(translate_pearlite(*right, top_ctx, idents.clone())))
//         }
...
//             translate_pearlite(*func, top_ctx, idents.clone())
//                 .append(RcDoc::space())
//                 .append(RcDoc::intersperse(
//                     args.into_iter()
...
//             RcDoc::as_string("nth")
//                 .append(RcDoc::space())
//                 .append(make_paren(translate_pearlite(*expr, top_ctx, idents.clone())))
//                 .append(RcDoc::space())
//                 .append(make_paren(translate_pearlite(*index, top_ctx, idents.clone())))
...
//             //     Expression::MethodCall(_, _, TopLevelIdent( format!("{}", method) ), _),
//             //     top_ctx
//             // )
            
//             // RcDoc::as_string("TODOMethodCall")
//             RcDoc::as_string(format!("{}", method))
//                 .append(RcDoc::space())
//                 .append(make_paren(translate_pearlite(*receiver, top_ctx, idents.clone())))
//                 .append(RcDoc::space())
//                 .append(RcDoc::intersperse(
//                     args.into_iter()
//                         .map(|x| make_paren(translate_pearlite(x, top_ctx, idents.clone()))),
...
//             make_paren(translate_pearlite(*expr, top_ctx, idents.clone()))
//         }
...
//                 match idents.iter().find(|(x, _)| x.clone() == id.to_string()) {
//                     Some((_, ident)) => rc.append(translate_ident(ident.clone())),
//                     None => rc.append(translate_ident_str(id.to_string())),
//                 }
//             }
//         }),
...
//             translate_pearlite(*term, top_ctx, idents.clone()) // Model supported? (@)
//         }
...
//             make_paren(translate_pearlite(*lhs, top_ctx, idents.clone()))
//                 .append(RcDoc::space())
//                 .append(RcDoc::as_string("="))
//                 .append(RcDoc::space())
//                 .append(make_paren(translate_pearlite(*rhs, top_ctx, idents.clone())))
//         }
...
//                 .append(RcDoc::space())
//                 .append(RcDoc::intersperse(x
//                     args.iter().map(|x| RcDoc::as_string(x.ident.to_string())),
//                     RcDoc::space()
//                 ))
//                 .append(RcDoc::as_string(","))
//                 .append(RcDoc::space())
...
fn translate_quantified_expression<'a>(qe: Quantified<(Ident, Spanned<BaseTyp>), Spanned<Expression>>, top_ctx: &'a TopLevelContext) -> RcDoc<'a, ()> {
    match qe {
        ...
        Quantified::Exists(ids, qe2) =>
            ...
            .append(RcDoc::intersperse(ids.iter().map(|(x, _)| translate_ident(x.clone())), RcDoc::space()))
            .append(translate_quantified_expression(*qe2, top_ctx)),
        Quantified::Implication(qe2, qe3) =>
...
fn translate_item<'a>(
    ...
    match &item.item {
        Item::FnDecl((f, _), sig, (b, _), requires, ensures) => make_let_binding( // requires, ensures, idents
            ...
            // .append(if item.tags.0.contains(&ItemTag::Tag("test".to_string())) {
            //     RcDoc::hardline()
            //         .append(RcDoc::hardline())
        ...
        Item::EnumDecl(name, cases) => RcDoc::as_string("Inductive")
            ...
            .append(RcDoc::as_string("."))
            .append(if item.tags.0.contains(&ItemTag::Tag("PartialEq".to_string())) {
                RcDoc::hardline()
                    .append(RcDoc::hardline())
                    .append(
                        RcDoc::as_string("Definition")
                    ...
                    .append(RcDoc::intersperse(
                        cases.into_iter().map(|(case_name, case_typ)| {
                            ...
                            translate_enum_case_name(
                                ...
                                .append(match case_typ {
                                    None => RcDoc::nil(),
                                    Some(_) => RcDoc::as_string("append").append(RcDoc::space()),
                                })
                                .append(RcDoc::as_string("\""))
                                .append(
                                    translate_enum_case_name(
...
pub fn translate_and_write_to_file(
    ...
    let export_quick_check = p
        .items
        .iter()
        .any(|i| i.0.tags.0.contains(&ItemTag::Tag("quickcheck".to_string())));
    write!(
        file,
