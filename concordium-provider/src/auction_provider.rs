...
fn auction_bid<A: HasActions>(
    ...
    let (state, res) = auction::auction_bid(their_context_to_my_context(ctx), amount.micro_gtu, their_state_to_my_state(state));
    
    match res {
        Ok(a) => Ok(A::accept()),
        Err(e) => Err(match e {
            auction::BidError::ContractSender                        => BidError::ContractSender,
            auction::BidError::BidTooLow                             => BidError::BidTooLow,
            auction::BidError::BidsOverWaitingForAuctionFinalization => BidError::BidsOverWaitingForAuctionFinalization,
            auction::BidError::AuctionFinalized                      => BidError::AuctionFinalized,
        }),
    }
}
